...
abstract interface class IReceiptLocalRepository {
  ...
  Future<void> save(Receipt receipt, {bool markPending});
...
/// HARD delete, local only — drops the row with NO tombstone.
  ...
  Future<List<Receipt>> getAllIncludingDeleted();

  /// Rows whose `syncStatus` is not `synced`. Drives the push pass.
  Future<List<Receipt>> getPending();
  ...
  Future<void> saveAll(List<Receipt> items, {bool markPending});
