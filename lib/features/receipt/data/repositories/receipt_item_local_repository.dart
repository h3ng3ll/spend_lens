...
class ReceiptItemLocalRepository implements IReceiptItemLocalRepository {
  ...
  Future<void> save(ReceiptItem item, {bool markPending = true}) async {
    final box = await _hiveDatabase.getBox<ReceiptItem>(_boxName);
    await box.put(item.id, await _stamped(box, item, markPending));
  }

  @override
  Future<void> saveAll(List<ReceiptItem> items, {bool markPending = true}) async {
    final box = await _hiveDatabase.getBox<ReceiptItem>(_boxName);
    final entries = <String, ReceiptItem>{};
  ...
  @override
  Future<List<ReceiptItem>> getPending() async {
    final box = await _hiveDatabase.getBox<ReceiptItem>(_boxName);
    return box.values
  ...
  /// places that forgot.
  bool _isVisible(ReceiptItem entity) => entity.deletedAt == null;
