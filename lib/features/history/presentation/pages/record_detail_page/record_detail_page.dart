...
///
/// The header carries NO Edit action. This build's `init_router.dart` has
/// no dedicated edit-cash-expense route (only `CashExpensePageRoute`, which
/// CREATES a new expense, was found), and inventing a new route/screen for
/// it is outside this task's scope (CLAUDE.md's "no invented screens/
/// routes/flows" rule). A control that only echoes its own label back as a
/// toast is a dead control shipped as if it worked — removing it is the
/// correct minimal fix (CLAUDE.md's "a defect the design causes is still a
/// bug" carve-out), not scope creep, since no new screen/route/flow is
/// added.
///
